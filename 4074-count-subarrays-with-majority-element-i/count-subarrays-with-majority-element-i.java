...
class Solution {
    ...
    private boolean isMajority(HashMap<Integer, Integer> freq,
                               int target,
                               int len) {

        return freq.getOrDefault(target, 0) > len / 2;
    }
    ...
    public int countMajoritySubarrays(int[] nums, int target) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = i; j < n; j++) {
                ...
                freq.put(nums[j],
                        freq.getOrDefault(nums[j], 0) + 1);

                int len = j - i + 1;

                if (isMajority(freq, target, len)) {
                    count++;
                }
            }
        }
